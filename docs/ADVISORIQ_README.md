...
The website now includes a complete client-side sample screener. It does not just display handpicked cards. On page load, `advisoriq/src/advisoriq-app.js` runs the screener and:
...
For this MVP, Indian stock and ETF data is stored locally in `advisoriq/src/advisoriq-app.js`. The monthly picks are no longer manually selected cards; they are generated by the client-side screening engine from the local sample universe.
...
```text
advisoriq/src/advisoriq-app.js
```
...
Conflicts usually happen when the same file and same lines are changed in two places: for example, you edit `advisoriq/src/advisoriq-app.js` locally while someone else also edits the same `stocks` array on GitHub.
...
- Put future real stock data in separate data files instead of one large `advisoriq/src/advisoriq-app.js` file. That will reduce conflicts later.

## Recommended next refactor

To reduce future Git conflicts, split `advisoriq/src/advisoriq-app.js` like this:
