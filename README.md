...
The website now includes a complete client-side sample screener. It does not just display handpicked cards. On page load, `src/app.js` runs the screener and:
...
For this MVP, Indian stock and ETF data is stored locally in `src/app.js`. The monthly picks are no longer manually selected cards; they are generated by the client-side screening engine from the local sample universe.
